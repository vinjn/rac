...
#include "MyWheeledVehicleMovementComponent4W.generated.h"

UENUM()
namespace EMyVehicleDifferential4W
{
	enum Type
	{
		LimitedSlip_4W,
...
USTRUCT()
struct FMyVehicleDifferential4WData
{
	GENERATED_BODY()

	FMyVehicleDifferential4WData();

	/** Type of differential */
	UPROPERTY(EditAnywhere, Category=Setup)
	TEnumAsByte<EMyVehicleDifferential4W::Type> DifferentialType;
...
USTRUCT()
struct FMyVehicleEngineData
{
	GENERATED_BODY()

	FMyVehicleEngineData();
...
struct FMyVehicleTransmissionData
{
	...
	TArray<FMyVehicleGearData> ForwardGears;
...
class  UMyWheeledVehicleMovementComponent4W : public UMyWheeledVehicleMovementComponent
{
	...
	FMyVehicleEngineData EngineSetup;

	/** Differential */
	UPROPERTY(EditAnywhere, Category = MechanicalSetup)
	FMyVehicleDifferential4WData DifferentialSetup;
	...
	/** Transmission data */
	UPROPERTY(EditAnywhere, Category = MechanicalSetup)
	FMyVehicleTransmissionData TransmissionSetup;
...
protected:
	...
	void UpdateEngineSetup(const FMyVehicleEngineData& NewEngineSetup);

	/** update simulation data: differential */
	void UpdateDifferentialSetup(const FMyVehicleDifferential4WData& NewDifferentialSetup);

	/** update simulation data: transmission */
	void UpdateTransmissionSetup(const FMyVehicleTransmissionData& NewGearSetup);
};
