...
UENUM()
enum EMyWheelSweepType
{
	/** Sweeps against both simple and complex geometry. */
	SimplePlusComplex UMETA(DisplayName="SimplePlusComplex"),	
	/** Sweeps against simple geometry only */
	Simple_	UMETA(DisplayName="Simple_"),		
	/** Sweeps against complex geometry only */
	Complex_	UMETA(DisplayName="Complex_")	
};

class UMyVehicleWheel;
...
class MYPHYSXVEHICLES_API UMyVehicleWheel : public UObject
{
	...
	class UMyTireConfig*								TireConfig;
	...
	UPROPERTY(EditAnywhere, Category = Suspension)
	TEnumAsByte<EMyWheelSweepType> SweepType;
	...
	UPROPERTY(transient)
	class UMyWheeledVehicleMovementComponent*			VehicleSim;
	 ...
	 */
	virtual void Init( class UMyWheeledVehicleMovementComponent* InVehicleSim, int32 InWheelIndex );
	 ...
	 */
	struct FMyWheelSetup& GetWheelSetup();
...
private:
	...
	FMyPhysXVehicleManager* GetVehicleManager() const;
#endif // WITH_PHYSX
