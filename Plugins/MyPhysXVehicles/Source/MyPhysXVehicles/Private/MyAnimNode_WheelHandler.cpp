...
FMyAnimNode_WheelHandler::FMyAnimNode_WheelHandler()
{
	AnimInstanceProxy = nullptr;
}

void FMyAnimNode_WheelHandler::GatherDebugData(FNodeDebugData& DebugData)
{
	...
	const TArray<FMyWheelAnimData>& WheelAnimData = AnimInstanceProxy->GetWheelAnimData();
	for (const FMyWheelLookupData& Wheel : Wheels)
	{
...
void FMyAnimNode_WheelHandler::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
...
bool FMyAnimNode_WheelHandler::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones) 
{
...
void FMyAnimNode_WheelHandler::InitializeBoneReferences(const FBoneContainer& RequiredBones) 
{
	const TArray<FMyWheelAnimData>& WheelAnimData = AnimInstanceProxy->GetWheelAnimData();
	...
	{
		FMyWheelLookupData* Wheel = new(Wheels)FMyWheelLookupData();
	...
	Wheels.Sort([](const FMyWheelLookupData& L, const FMyWheelLookupData& R) { return L.BoneReference.BoneIndex < R.BoneReference.BoneIndex; });
}

void FMyAnimNode_WheelHandler::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
	AnimInstanceProxy = (FMyVehicleAnimInstanceProxy*)Context.AnimInstanceProxy;	//TODO: This is cached for now because we need it in eval bone transforms.
}
