...
bool FMyPhysXVehicleManager::bUpdateTireFrictionTable = false;
PxVehicleDrivableSurfaceToTireFrictionPairs* FMyPhysXVehicleManager::SurfaceTirePairs = NULL;
TMap<FPhysScene*, FMyPhysXVehicleManager*> FMyPhysXVehicleManager::SceneToVehicleManagerMap;
uint32 FMyPhysXVehicleManager::VehicleSetupTag = 0;
...
FMyPhysXVehicleManager::FMyPhysXVehicleManager(FPhysScene* PhysScene)
...
#endif
{
	...
	// Set up delegates
	OnPhysScenePreTickHandle = PhysScene->OnPhysScenePreTick.AddRaw(this, &FMyPhysXVehicleManager::PreTick);
	OnPhysSceneStepHandle = PhysScene->OnPhysSceneStep.AddRaw(this, &FMyPhysXVehicleManager::Update);

	// Add to map
	FMyPhysXVehicleManager::SceneToVehicleManagerMap.Add(PhysScene, this);
...
void FMyPhysXVehicleManager::DetachFromPhysScene(FPhysScene* PhysScene)
{
	PhysScene->OnPhysScenePreTick.Remove(OnPhysScenePreTickHandle);
	PhysScene->OnPhysSceneStep.Remove(OnPhysSceneStepHandle);

	FMyPhysXVehicleManager::SceneToVehicleManagerMap.Remove(PhysScene);
}

FMyPhysXVehicleManager::~FMyPhysXVehicleManager()
{
#if PX_DEBUG_VEHICLE_ON
	if(TelemetryData4W)
	{
...
FMyPhysXVehicleManager* FMyPhysXVehicleManager::GetVehicleManagerFromScene(FPhysScene* PhysScene)
{
	FMyPhysXVehicleManager* Manager = nullptr;
	FMyPhysXVehicleManager** ManagerPtr = SceneToVehicleManagerMap.Find(PhysScene);
...
static UMyTireConfig* DefaultTireConfig = nullptr;

UMyTireConfig* FMyPhysXVehicleManager::GetDefaultTireConfig()
{
	if (DefaultTireConfig == nullptr)
	{
		DefaultTireConfig = NewObject<UMyTireConfig>();
...
void FMyPhysXVehicleManager::UpdateTireFrictionTable()
{
	bUpdateTireFrictionTable = true;
}

void FMyPhysXVehicleManager::UpdateTireFrictionTableInternal()
{
	...
	uint32 NumTireConfigs = UMyTireConfig::AllTireConfigs.Num();
	...
	for ( uint32 m = 0; m < NumMaterials; ++m )
	{
		...
		{
				UMyTireConfig* TireConfig = UMyTireConfig::AllTireConfigs[t].Get();
...
void FMyPhysXVehicleManager::SetUpBatchedSceneQuery()
{
	int32 NumWheels = 0;
...
void FMyPhysXVehicleManager::AddVehicle( TWeakObjectPtr<UMyWheeledVehicleMovementComponent> Vehicle )
{
	check(Vehicle != NULL);
...
void FMyPhysXVehicleManager::RemoveVehicle( TWeakObjectPtr<UMyWheeledVehicleMovementComponent> Vehicle )
{
	check(Vehicle != NULL);
	check(Vehicle->PVehicle);
...
void FMyPhysXVehicleManager::Update(FPhysScene* PhysScene, float DeltaTime)
{
...
void FMyPhysXVehicleManager::PreTick(FPhysScene* PhysScene, float DeltaTime)
{
...
void FMyPhysXVehicleManager::UpdateVehicles( float DeltaTime )
{
...
PxVec3 FMyPhysXVehicleManager::GetSceneGravity_AssumesLocked()
{
	return Scene->getGravity();
}

void FMyPhysXVehicleManager::SetRecordTelemetry( TWeakObjectPtr<UMyWheeledVehicleMovementComponent> Vehicle, bool bRecord )
{
#if PX_DEBUG_VEHICLE_ON
...
void FMyPhysXVehicleManager::SetupTelemetryData()
{
...
void FMyPhysXVehicleManager::UpdateVehiclesWithTelemetry( float DeltaTime )
{
...
PxVehicleTelemetryData* FMyPhysXVehicleManager::GetTelemetryData_AssumesLocked()
{
	if ( TelemetryVehicle )
	{
...
PxWheelQueryResult* FMyPhysXVehicleManager::GetWheelsStates_AssumesLocked(TWeakObjectPtr<const UMyWheeledVehicleMovementComponent> Vehicle)
{
