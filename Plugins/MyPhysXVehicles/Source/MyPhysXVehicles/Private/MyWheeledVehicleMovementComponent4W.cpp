...
UMyWheeledVehicleMovementComponent4W::UMyWheeledVehicleMovementComponent4W(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
#if PHYSICS_INTERFACE_PHYSX
	// grab default values from physx
	PxVehicleDifferential4WData DefDifferentialSetup;
	TEnumAsByte<EMyVehicleDifferential4W::Type> DiffType((uint8)DefDifferentialSetup.mType);
...
#if WITH_EDITOR
void UMyWheeledVehicleMovementComponent4W::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == TEXT("DownRatio"))
	{
		...
		{
			FMyVehicleGearData & GearData = TransmissionSetup.ForwardGears[GearIdx];
...
static void GetVehicleDifferential4WSetup(const FMyVehicleDifferential4WData& Setup, PxVehicleDifferential4WData& PxSetup)
{
...
FMyVehicleEngineData::FMyVehicleEngineData()
	: MaxRPM(0.0f)
	, MOI(0.0f)
...
float FMyVehicleEngineData::FindPeakTorque() const
{
	// Find max torque
...
#if PHYSICS_INTERFACE_PHYSX
static void GetVehicleEngineSetup(const FMyVehicleEngineData& Setup, PxVehicleEngineData& PxSetup)
{
...
static void GetVehicleGearSetup(const FMyVehicleTransmissionData& Setup, PxVehicleGearsData& PxSetup)
{
...
static void GetVehicleAutoBoxSetup(const FMyVehicleTransmissionData& Setup, PxVehicleAutoBoxData& PxSetup)
{
	for (int32 i = 0; i < Setup.ForwardGears.Num(); i++)
	{
		const FMyVehicleGearData& GearData = Setup.ForwardGears[i];
...
void SetupDriveHelper(const UMyWheeledVehicleMovementComponent4W* VehicleData, const PxVehicleWheelsSimData* PWheelsSimData, PxVehicleDriveSimData4W& DriveData)
{
...
void UMyWheeledVehicleMovementComponent4W::SetupVehicleDrive(PxVehicleWheelsSimData* PWheelsSimData)
{
...
void UMyWheeledVehicleMovementComponent4W::UpdateSimulation(float DeltaTime)
{
...
void UMyWheeledVehicleMovementComponent4W::UpdateEngineSetup(const FMyVehicleEngineData& NewEngineSetup)
{
#if PHYSICS_INTERFACE_PHYSX
	if (PVehicleDrive)
	{
...
void UMyWheeledVehicleMovementComponent4W::UpdateDifferentialSetup(const FMyVehicleDifferential4WData& NewDifferentialSetup)
{
#if PHYSICS_INTERFACE_PHYSX
	if (PVehicleDrive)
	{
...
void UMyWheeledVehicleMovementComponent4W::UpdateTransmissionSetup(const FMyVehicleTransmissionData& NewTransmissionSetup)
{
#if PHYSICS_INTERFACE_PHYSX
	if (PVehicleDrive)
	{
...
void UMyWheeledVehicleMovementComponent4W::Serialize(FArchive & Ar)
{
	Super::Serialize(Ar);
#if PHYSICS_INTERFACE_PHYSX
...
void UMyWheeledVehicleMovementComponent4W::ComputeConstants()
{
	Super::ComputeConstants();
	MaxEngineRPM = EngineSetup.MaxRPM;
}

FMyVehicleTransmissionData::FMyVehicleTransmissionData()
...
FMyVehicleGearData::FMyVehicleGearData()
	: Ratio(1.0f)
	, DownRatio(0.0f)
	, UpRatio(1.0f)
{
...
FMyVehicleDifferential4WData::FMyVehicleDifferential4WData()
	: DifferentialType(EMyVehicleDifferential4W::LimitedSlip_4W)
	, FrontRearSplit(0.5f)
